    ...
    def crawl(self, dirname):
        ...
        for file in glob.glob(pr + "*.pdf"):
            ...
            with open(url, 'rb') as f:
                ...
                countCurr = 0
                while countCurr < pagecount:
                    pageObj = pdfread.getPage(countCurr)
                    countCurr += 1
...
dirname = input("Please enter your directory(full path. e.g.- /home/<pc_name>/<directory>...) : \n")
if not os.path.isdir(dirname):
    print("Given directory doesn't exist. Please give correct path")
    exit(0);
filename = dirname.replace('/', '+')
docfile = filename + "doclist.txt"
wordfile = filename + "wordfile.txt"
wordlocfile = filename + "wordlocfile.txt"
...
while permission:
    ...
    query = input('Please enter your query (enclose under "."(double-quotes) for full-phrase query)....')
    if query != '':
